...
export class PurchasePrintComponent implements OnInit, OnDestroy {
  ...
  company: Company;
  get isTaxPurchase(): boolean {
    return !this.purchase.hideTax;
  }
  ...
  getTotalQuantity(): number {
    return this.purchase.items.reduce((a, b) => a + (b.quantity || 0), 0);
  }

  getRoundoff(): number {
    return this.purchase.total - Math.floor(this.purchase.total);
  }

  getTotal(): number {
    return Math.floor(this.purchase.total);
  }
